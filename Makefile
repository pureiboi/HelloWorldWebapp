...
integration_test:
	docker-compose  -f ./deployments/docker-compose.yml -v up -d

stop_integration_test:
	docker-compose  -f ./deployments/docker-compose.yml -v down

docker_build:
	docker build -f ./deployments/build/Dockerfile -o type=local,dest=bin/pinger -o type=image,name=devops/pinger:latest .
